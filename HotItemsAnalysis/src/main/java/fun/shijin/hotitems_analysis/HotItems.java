package fun.shijin.hotitems_analysis;

import fun.shijin.hotitems_analysis.beans.ItemViewCount;
import fun.shijin.hotitems_analysis.beans.UserBehavior;
...
public class HotItems {
    public static void main(String[] args) throws Exception {
        ...
        // 2. 读取数据，创建DataStream
//        DataStream<String> inputStream = env.readTextFile("F:\\Demo\\Flink\\UserBehaviorAnalysis\\HotItemsAnalysis\\src\\main\\resources\\UserBehavior.csv");

        Properties properties = new Properties();
        properties.setProperty("bootstrap.servers", "hadoop111:9092");
    ...
    private static class TopNHotItems extends KeyedProcessFunction<Tuple, ItemViewCount, String> {
        ...
        public void open(Configuration parameters) throws Exception {
            itemViewCountListState = getRuntimeContext().getListState(new ListStateDescriptor<ItemViewCount>("item-view-count-state", ItemViewCount.class));
        }

        @Override
        public void processElement(ItemViewCount value, Context context, Collector<String> collector) throws Exception {
            // 每来一个数据就存入list中，并注册定时器
